...
densities  = [3, 2, 1, 0]
velocities = [0.05, 0., -0.05]
angles     = [0, pi / 4, 3 * pi / 4, pi]
...
def compute( s1, s2, frame ):
  ...
  for o in frame:
    dist  = momo.distance( agt_vel, o[:2] )
    r_vel   = o[2:] - agt_vel
    if dist < 4:
      tot_density += 1
      sum_vel += r_vel
  if tot_density > 0:
    sum_vel /= tot_density
    r_angle = abs( momo.as_angle( sum_vel ) )
  ...
  result[n_density + n_velocity * n_angle + 1] = momo.distance( s1[:2], s2[:2] )
  return result
